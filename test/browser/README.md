...
4. Captures browser console messages — any `console.error` or uncaught exception
   is logged.
5. Polls the server `/health` endpoint to confirm the server is running.
6. Exercises the core gameplay loop:
   ...
   - End turn action fires
7. On any step timeout, captures a screenshot and fails with a descriptive
   message.
...
To skip in CI environments without a browser:

```bash
go test ./test/browser/... -short
```

### Screenshots

Step screenshots are saved to `test/browser/screenshots/` (`.png` files are
gitignored, only `.gitkeep` is tracked).

### Requirements

- Google Chrome or Chromium must be installed and in `$PATH`
