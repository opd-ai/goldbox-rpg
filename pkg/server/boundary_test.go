...
func TestMovementBoundaryEnforcement(t *testing.T) {
	...
	t.Run("Movement beyond north boundary should be prevented", func(t *testing.T) {
		...
		// Starting at Y=2, should be able to move to Y=3, Y=4, but no further
		...
		if player.GetPosition().Y != 3 {
			t.Errorf("Expected Y=3 after first north move, got Y=%d", player.GetPosition().Y)
		}
		...
		if player.GetPosition().Y != 4 {
			t.Errorf("Expected Y=4 after second north move, got Y=%d", player.GetPosition().Y)
		}
		...
		if player.GetPosition().Y != 4 {
			t.Errorf("Expected Y=4 after third north move (clamped), got Y=%d", player.GetPosition().Y)
		}
		...
		if position.Y != 4 {
			t.Errorf("Expected result position Y=4, got Y=%d", position.Y)
		}
	})
	...
	t.Run("Movement beyond south boundary should be prevented", func(t *testing.T) {
		...
		if player.GetPosition().Y != 1 {
			t.Errorf("Expected Y=1 after first south move, got Y=%d", player.GetPosition().Y)
		}
		...
		if player.GetPosition().Y != 0 {
			t.Errorf("Expected Y=0 after second south move, got Y=%d", player.GetPosition().Y)
		}

		// Try to move below Y=0 (should be prevented)
		...
		if player.GetPosition().Y != 0 {
			t.Errorf("Expected Y=0 after third south move (clamped), got Y=%d", player.GetPosition().Y)
		}
	})
