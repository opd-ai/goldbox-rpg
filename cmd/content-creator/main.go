...
func outputSpell(spell game.Spell, outputFile string) error {
	data, err := yaml.Marshal(spell)
	if err != nil {
		return fmt.Errorf("failed to marshal spell: %w", err)
	}

	output := fmt.Sprintf("# Spell generated by content-creator\n# ID: %s\n# Name: %s\n\n%s",
		spell.ID, spell.Name, string(data))
...
func outputItem(item game.Item, outputFile string) error {
	...
	output := fmt.Sprintf("# Item generated by content-creator\n# ID: %s\n# Name: %s\n\n%s",
		item.ID, item.Name, string(data))
