...
class CombatManager extends EventEmitter {
  ...
  constructor(gameState, renderer) {
    ...
    try {
      console.debug("CombatManager.constructor: params", {
        gameState,
        renderer,
      });
  ...
  cleanup() {
    ...
    try {
      ...
      if (this.highlightedCells.size > 0) {
        console.warn(
          "CombatManager.cleanup: clearing non-empty highlighted cells",
        );
      }
  ...
  setupEventListeners() {
    ...
    try {
      console.debug(
        "CombatManager.setupEventListeners: initializing event listeners",
      );
      ...
      document.querySelectorAll(".action-btn").forEach((btn) => {
        btn.addEventListener("click", () => {
          console.info(
            "CombatManager.setupEventListeners: action button clicked",
            btn.dataset.action,
          );
      ...
      console.info(
        "CombatManager.setupEventListeners: attached action button listeners",
      );
  ...
  async handleActionButton(action) {
    ...
    try {
      ...
      if (!this.active || this.currentTurn !== this.gameState.player.id) {
        console.warn(
          "CombatManager.handleActionButton: action blocked - inactive or not player turn",
        );
        console.groupEnd();
        return;
      }
      ...
      console.info(
        "CombatManager.handleActionButton: selected new action",
        action,
      );

      this.highlightValidTargets(action);
      console.info(
        "CombatManager.handleActionButton: highlighted valid targets",
      );
    } catch (err) {
  ...
  async playActionAnimation(action, target, result) {
    ...
    try {
      console.debug("CombatManager.playActionAnimation: params", {
        action,
        target,
        result,
      });
      ...
      switch (action) {
        case "attack":
          console.info(
            "CombatManager.playActionAnimation: playing attack animation",
          );
          ...
          if (result.hit) {
            console.info(
              "CombatManager.playActionAnimation: playing damage number animation",
            );
            await this.renderer.playDamageNumber(
              target.position,
              result.damage,
            );
          } else {
        ...
        case "cast":
          console.info(
            "CombatManager.playActionAnimation: playing spell animation",
          );
        ...
        default:
          console.warn(
            "CombatManager.playActionAnimation: unhandled action type",
            action,
          );
      }
    } catch (err) {
  ...
  highlightValidTargets(action) {
    ...
    try {
      ...
      this.highlightedCells.clear();
      console.info(
        "CombatManager.highlightValidTargets: cleared highlighted cells",
      );
      ...
      switch (action) {
        case "attack":
          ...
          console.info(
            "CombatManager.highlightValidTargets: highlighted attack targets",
          );
          break;
        case "cast":
          this.highlightSpellTargets();
          console.info(
            "CombatManager.highlightValidTargets: highlighted spell targets",
          );
          break;
        case "item":
          this.highlightItemTargets();
          console.info(
            "CombatManager.highlightValidTargets: highlighted item targets",
          );
          break;
        default:
          console.warn(
            "CombatManager.highlightValidTargets: unrecognized action type",
            action,
          );
      }
      ...
      this.renderer.updateHighlights(this.highlightedCells);
      console.info(
        "CombatManager.highlightValidTargets: updated renderer highlights",
      );
    } catch (err) {
  ...
  highlightAttackTargets() {
    ...
    try {
      ...
      console.debug("CombatManager.highlightAttackTargets: params", {
        range,
        playerPos,
      });
      ...
      if (targetCount === 0) {
        console.warn(
          "CombatManager.highlightAttackTargets: no valid targets found in range",
        );
      } else {
        console.info(
  ...
  highlightSpellTargets() {
    ...
    try {
      ...
      const playerPos = this.gameState.player.position;
      console.debug("CombatManager.highlightSpellTargets: params", {
        range,
        playerPos,
      });
      ...
      if (targetCount === 0) {
        console.warn(
          "CombatManager.highlightSpellTargets: no valid targets found in range",
        );
      } else {
        console.info(
  ...
  highlightItemTargets() {
    ...
    try {
      ...
      const playerPos = this.gameState.player.position;
      console.debug("CombatManager.highlightItemTargets: params", {
        range,
        playerPos,
      });
      ...
      if (targetCount === 0) {
        console.warn(
          "CombatManager.highlightItemTargets: no valid targets found in range",
        );
      } else {
        console.info(
  ...
  getGridPosition(event) {
    ...
    try {
      ...
      if (event.clientX < rect.left || event.clientY < rect.top) {
        console.warn(
          "CombatManager.getGridPosition: click outside grid bounds",
        );
      }

      const x = Math.floor(
        (event.clientX - rect.left) / this.renderer.tileSize,
      );
      const y = Math.floor((event.clientY - rect.top) / this.renderer.tileSize);
      console.info("CombatManager.getGridPosition: calculated grid coords", {
        x,
        y,
      });
