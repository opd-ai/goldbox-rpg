...
func TestBrowserPlaytest(t *testing.T) {
	...
	healthResp, err := healthClient.Get(ts.baseURL + "/health")
	if err != nil || healthResp.StatusCode != 200 {
		saveScreenshot(ctx, t, "04-health-fail")
		t.Fatalf("health endpoint failed: err=%v status=%v", err, healthResp)
	}
...
func checkJSErrors(ctx context.Context, t *testing.T, step string) {
	...
	if len(critical) > 0 {
		...
		// are informational. The test already validates canvas presence and
		// server health.
